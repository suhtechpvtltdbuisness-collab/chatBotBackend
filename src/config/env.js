...
export const loadEnv = () => {
  ...
  if (result.error) {
    console.warn(
      `⚠️ Could not load .env from ${envPath}. Falling back to process environment.`,
    );
  }
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      if (!process.env.MONGO_URI)
        process.env.MONGO_URI =
          "mongodb+srv://Ankit:Ankit@cluster0.m609d.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";
    }
  }
...
export const config = {
  mongodb: {
    uri:
      process.env.MONGO_URI ||
      "mongodb+srv://User:Akp%40123@cluster0.v5cjcdr.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0",
  },
  jwt: {
  ...
  llm: {
    ...
    allowStubFallback:
      (process.env.ALLOW_LLM_STUB_FALLBACK || "false").toLowerCase() === "true",
  },
  server: {
