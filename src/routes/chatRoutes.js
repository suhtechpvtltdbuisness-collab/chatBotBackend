...
import { authenticateApiKey, requirePermission, trackUsage } from '../middleware/tenant.js';
...
router.post('/start', authenticateApiKey, requirePermission('chat:write'), trackUsage('apiCalls'), [
...
router.post('/message', authenticateApiKey, requirePermission('chat:write'), trackUsage('apiCalls'), [
  ...
  try {
    ...
    try {
      const external = await externalContextService.fetchContextForMessage(req.tenant, {
        query: userMessage,
        sessionId,
        visitor: conversation.visitor,
      });
      ...
      console.warn('External context integration error:', extErr?.message || extErr);
    }
...
router.get('/:sessionId/history', authenticateApiKey, requirePermission('chat:read'), trackUsage('apiCalls'), async (req, res) => {
  try {
...
router.get('/admin/:conversationId/history', authenticateToken, async (req, res) => {
  try {
    ...
    const messages = await Message.getConversationHistory(conversation._id, 200);
...
// End conversation
router.post('/:sessionId/end', authenticateApiKey, requirePermission('chat:write'), trackUsage('apiCalls'), [
...
router.post('/:sessionId/handoff', authenticateApiKey, requirePermission('chat:write'), trackUsage('apiCalls'), [
...
router.post('/:conversationId/agent/assign', authenticateToken, async (req, res) => {
  try {
    const { conversationId } = req.params;
    const agentId = req.user._id.toString();
    const io = req.app.get('io');
    ...
    if (!conversation || conversation.tenantId.toString() !== req.tenant._id.toString()) {
      return res.status(404).json({ error: 'Conversation not found' });
    }
    ...
    if (conversation.assignedAgent) {
      ...
      if (conversation.assignedAgent.toString() !== agentId) {
        return res.status(409).json({ error: 'Conversation already assigned to another agent' });
      }
      return res.json({ success: true, assigned: true });
    }

    // Assign and set status
    await conversation.assignToAgent(agentId, { reason: 'Agent claimed conversation', priority: 'medium' });

    // Send intro message and emit join event
    await handoffService.addAgentMessage(conversationId, agentId, `Hi! I'm ${req.user.name}, I'll help you now.`);
    if (io) {
